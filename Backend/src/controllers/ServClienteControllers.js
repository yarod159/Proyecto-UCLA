...
const SolGarantia = require("../models/SolGarantia");
...
const deleteReporteCliente = async (req, res) => {
 ...
 const postSolGarantia = async (req, res) => {
  try {
    // Crear una nueva instancia de Empleado con los datos del formulario
    const solGarantia = new SolGarantia({
      name: req.body.name,
      email: req.body.email,
      phone: req.body.phone,
      fecha: req.body.fecha,
...
const getSolGarantia = async (req, res) => {
  try {
    const solGarantia = await SolGarantia.find();
    res.status(200).json({
      success: true,
      data: solGarantia,
    });
...
const deleteSolGarantia = async (req, res) => {
  try {
     // Asegúrate de que el ID de la FAQ se envía en la URL o en el cuerpo de la solicitud
     const solGarantiaId = req.params.id; // Si el ID se envía como parámetro en la URL
     // const faqId = req.body.id; // Si el ID se envía en el cuerpo de la solicitud
 
     // Buscar y eliminar la FAQ por su ID
     const result = await ReporteCliente.findByIdAndDelete(solGarantiaId);
...
module.exports = { postReporteCliente, getReporteCliente, deleteReporteCliente, postSolGarantia, getSolGarantia };
